...
function classifyAdaptiveWatermarkSignal(adaptiveResult) {
  ...
  const size = toFiniteNumber((_a = adaptiveResult == null ? void 0 : adaptiveResult.region) == null ? void 0 : _a.size);
...
function inferDecisionTier(candidate, { directMatch = false } = {}) {
  ...
  if ((_a = candidate.source) == null ? void 0 : _a.includes("validated")) return "validated-match";
...
function shouldSearchNearbyStandardCandidate(candidate, originalImageData) {
  var _a;
  if (!candidate) return true;
  return Number((_a = candidate.position) == null ? void 0 : _a.width) >= 72 && Number(originalImageData == null ? void 0 : originalImageData.height) > Number(originalImageData == null ? void 0 : originalImageData.width) * 1.25 && (Math.abs(candidate.processedSpatialScore) > STANDARD_NEARBY_SEARCH_RESIDUAL_THRESHOLD || Math.max(0, candidate.processedGradientScore) > STANDARD_NEARBY_SEARCH_GRADIENT_THRESHOLD);
}
...
function pickBestValidatedCandidate(candidates) {
  const accepted = candidates.filter((candidate) => candidate == null ? void 0 : candidate.accepted);
...
function pickBetterCandidate(currentBest, candidate, minCostDelta = 5e-3) {
  if (!(candidate == null ? void 0 : candidate.accepted)) return currentBest;
...
function isStandardCandidateSource(candidate) {
  return typeof (candidate == null ? void 0 : candidate.source) === "string" && candidate.source.startsWith("standard");
}
function isDriftedStandardCandidate(candidate) {
  var _a, _b;
  return isStandardCandidateSource(candidate) && (((_a = candidate == null ? void 0 : candidate.provenance) == null ? void 0 : _a.localShift) === true || ((_b = candidate == null ? void 0 : candidate.provenance) == null ? void 0 : _b.sizeJitter) === true || String((candidate == null ? void 0 : candidate.source) || "").includes("+warp"));
}
function isCanonicalStandardCandidate(candidate) {
  var _a, _b;
  return isStandardCandidateSource(candidate) && ((_a = candidate == null ? void 0 : candidate.provenance) == null ? void 0 : _a.localShift) !== true && ((_b = candidate == null ? void 0 : candidate.provenance) == null ? void 0 : _b.sizeJitter) !== true;
}
function hasStrongCanonicalAnchorSignal(candidate) {
  const baseSpatial = Number(candidate == null ? void 0 : candidate.originalSpatialScore);
  const baseGradient = Number(candidate == null ? void 0 : candidate.originalGradientScore);
...
function hasWeakDriftEvidence(candidate) {
  const candidateSpatial = Number(candidate == null ? void 0 : candidate.originalSpatialScore);
  const candidateGradient = Number(candidate == null ? void 0 : candidate.originalGradientScore);
...
function leavesWorseResidualGradientThanCanonical(canonicalCandidate, driftCandidate) {
  const canonicalProcessedGradientRaw = Number(canonicalCandidate == null ? void 0 : canonicalCandidate.processedGradientScore);
  const driftProcessedGradientRaw = Number(driftCandidate == null ? void 0 : driftCandidate.processedGradientScore);
...
function shouldPreserveStrongStandardAnchor(currentBest, candidate) {
  var _a;
  if (((_a = currentBest == null ? void 0 : currentBest.provenance) == null ? void 0 : _a.localShift) === true) return false;
...
function shouldRevertLocalShiftToStandardTrial(selectedCandidate, standardTrial) {
  var _a;
  if (((_a = selectedCandidate == null ? void 0 : selectedCandidate.provenance) == null ? void 0 : _a.localShift) !== true) return false;
  if (!isStandardCandidateSource(selectedCandidate) || !isStandardCandidateSource(standardTrial)) return false;
  if (!(standardTrial == null ? void 0 : standardTrial.accepted)) return false;
...
function isPreviewAnchorSearchEligible(originalImageData, config) {
  ...
  const width = Number(originalImageData == null ? void 0 : originalImageData.width);
  const height = Number(originalImageData == null ? void 0 : originalImageData.height);
...
function shouldPreferPreviewAnchorCandidate(currentBest, candidate) {
  var _a, _b;
  if (((_a = candidate == null ? void 0 : candidate.provenance) == null ? void 0 : _a.previewAnchor) !== true) return false;
  if (!currentBest || ((_b = currentBest == null ? void 0 : currentBest.provenance) == null ? void 0 : _b.previewAnchor) === true) return false;
  ...
  for (const seed of candidateSeeds) {
    ...
    for (const dy of STANDARD_NEARBY_SHIFTS) {
      for (const dx of STANDARD_NEARBY_SHIFTS) {
        ...
        if (!(candidate == null ? void 0 : candidate.accepted)) continue;
...
function searchFineStandardLocalCandidate({
  ...
  if (!(seedCandidate == null ? void 0 : seedCandidate.alphaMap) || !(seedCandidate == null ? void 0 : seedCandidate.position)) return null;
...
function evaluateStandardTrialsForSeeds({
  ...
  const standardSpatialScore = (standardTrial == null ? void 0 : standardTrial.originalSpatialScore) ?? null;
  const standardGradientScore = (standardTrial == null ? void 0 : standardTrial.originalGradientScore) ?? null;
...
function resolveCandidatePromotion(candidate, {
  ...
  if (!(candidate == null ? void 0 : candidate.accepted)) {
    return null;
  }
  if (reliableMatch) {
    return {
...
function evaluateAdaptiveTrial({
  ...
  const adaptiveConfidence = (adaptive == null ? void 0 : adaptive.confidence) ?? null;
  if (!(adaptive == null ? void 0 : adaptive.region) || !(hasReliableAdaptiveWatermarkSignal(adaptive) || adaptive.confidence >= VALIDATION_MIN_CONFIDENCE_FOR_ADAPTIVE_TRIAL)) {
    return {
      adaptive,
      adaptiveConfidence,
      adaptiveTrial: null
...
function refineSelectedAnchorCandidate({
  ...
  const warpCandidate = findBestTemplateWarp({
    ...
    shiftCandidates: ((_a = selectedTrial.provenance) == null ? void 0 : _a.previewAnchor) === true ? PREVIEW_TEMPLATE_ALIGN_SHIFTS : TEMPLATE_ALIGN_SHIFTS,
    scaleCandidates: ((_b = selectedTrial.provenance) == null ? void 0 : _b.previewAnchor) === true ? PREVIEW_TEMPLATE_ALIGN_SCALES : TEMPLATE_ALIGN_SCALES
  });
  if (warpCandidate) {
  ...
  const shouldRunGainSearch = ((_c = selectedTrial.provenance) == null ? void 0 : _c.previewAnchor) === true ? isPreviewAnchorGainSearchRequired(selectedTrial) : shouldEscalateSearch(selectedTrial);
...
function selectInitialCandidate({
  ...
  if (hasReliableStandardMatch && (standardTrial == null ? void 0 : standardTrial.accepted)) {
    baseCandidate = standardTrial;
    baseDecisionTier = "direct-match";
  } else if (standardTrial == null ? void 0 : standardTrial.accepted) {
    baseCandidate = {
      ...standardTrial,
  ...
  if (baseDecisionTier !== "direct-match" && !((_a = baseCandidate == null ? void 0 : baseCandidate.provenance) == null ? void 0 : _a.previewAnchor) && shouldEscalateSearch(baseCandidate)) {
  ...
  if (baseDecisionTier !== "direct-match" && ((_b = baseCandidate == null ? void 0 : baseCandidate.provenance) == null ? void 0 : _b.sizeJitter) === true && !((_c = baseCandidate == null ? void 0 : baseCandidate.provenance) == null ? void 0 : _c.previewAnchor) && isStandardCandidateSource(baseCandidate) && shouldEscalateSearch(baseCandidate)) {
  ...
  if (!((_d = baseCandidate == null ? void 0 : baseCandidate.provenance) == null ? void 0 : _d.previewAnchor) && !hasReliableAdaptiveWatermarkSignal(adaptive) && shouldSearchNearbyStandardCandidate(baseCandidate, originalImageData)) {
...
function createSelectionDebugSummary({
  ...
  return {
    ...
    usedCatalogVariant: ((_a = selectedTrial.provenance) == null ? void 0 : _a.catalogVariant) === true,
    usedSizeJitter: ((_b = selectedTrial.provenance) == null ? void 0 : _b.sizeJitter) === true,
    usedLocalShift: ((_c = selectedTrial.provenance) == null ? void 0 : _c.localShift) === true,
    usedAdaptive: ((_d = selectedTrial.provenance) == null ? void 0 : _d.adaptive) === true,
    usedPreviewAnchor: ((_e = selectedTrial.provenance) == null ? void 0 : _e.previewAnchor) === true
  };
}
...
function nowMs() {
  var _a;
  if (typeof ((_a = globalThis.performance) == null ? void 0 : _a.now) === "function") {
...
function refineSubpixelOutline({
  ...
  const baseDx = (baselineShift == null ? void 0 : baselineShift.dx) ?? 0;
  const baseDy = (baselineShift == null ? void 0 : baselineShift.dy) ?? 0;
  const baseScale = (baselineShift == null ? void 0 : baselineShift.scale) ?? 1;
  let best = null;
...
function shouldRefinePreviewResidualEdge({
  ...
  return typeof source === "string" && source.includes("preview-anchor") && (position == null ? void 0 : position.width) >= 24 && (position == null ? void 0 : position.width) <= PREVIEW_EDGE_CLEANUP_MAX_SIZE && (Math.abs(baselineSpatialScore) <= PREVIEW_EDGE_CLEANUP_SPATIAL_THRESHOLD || baselinePositiveHalo >= PREVIEW_EDGE_CLEANUP_STRONG_HALO_THRESHOLD && Math.abs(baselineSpatialScore) <= PREVIEW_EDGE_CLEANUP_HALO_SPATIAL_THRESHOLD) && baselineGradientScore >= PREVIEW_EDGE_CLEANUP_GRADIENT_THRESHOLD;
}
function shouldUsePreviewAnchorFastCleanup(selectedTrial, position) {
  var _a;
  return ((_a = selectedTrial == null ? void 0 : selectedTrial.provenance) == null ? void 0 : _a.previewAnchor) === true && (position == null ? void 0 : position.width) >= 24 && (position == null ? void 0 : position.width) <= PREVIEW_EDGE_CLEANUP_MAX_SIZE;
}
...
function processWatermarkImageData(imageData, options = {}) {
  ...
  const skipPreviewAnchorMultiPass = ((_a = selectedTrial == null ? void 0 : selectedTrial.provenance) == null ? void 0 : _a.previewAnchor) === true;
  ...
  finalImageData = (extraPassResult == null ? void 0 : extraPassResult.imageData) ?? finalImageData;
  passCount = (extraPassResult == null ? void 0 : extraPassResult.passCount) ?? 1;
  attemptedPassCount = (extraPassResult == null ? void 0 : extraPassResult.attemptedPassCount) ?? 1;
  passStopReason = (extraPassResult == null ? void 0 : extraPassResult.stopReason) ?? (firstPassClearedResidual ? "residual-low" : skipPreviewAnchorMultiPass ? "preview-anchor-single-pass" : "max-passes");
  passes = [firstPassRecord, ...(extraPassResult == null ? void 0 : extraPassResult.passes) ?? []];
  if (passCount > 1) {
...
//# sourceMappingURL=@pilio_gemini-watermark-remover_browser.js.map
